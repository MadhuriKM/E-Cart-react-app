    ...
    try {
       ...
       let outPath = path.join(__dirname, "../client/public/uploads")
        ...
        if(fs.existsSync(filePath)) {
            ...
            return res.status(StatusCode.CONFLICT).json({ status: false, msg: `File already exists.`})
        }

        // validate file type upload the file
        if(thumbnail.mimetype === "image/png" ||  thumbnail.mimetype === "image/jpeg" || thumbnail.mimetype === "image/jpg" || thumbnail.mimetype || "image/webp") {
            ...
            return res.status(StatusCodes.CONFLICT).json({ status: false, msg: "file type is not matched.. allows only .png, .jpg, .webp"})
        }
    } catch (err) {
...
const deleteImage = async (req,res) => {
    try {
       ...
       let extPro = await Product.findById({_id: product})
        ...
        // file already exusts in the location or not
        ...
        // update the product collection
         await Product.findByIdAndUpdate({ _id: product }, {image: { path: "https://t3.ftcdn.net/jpg/04/34/72/82/240_F_434728286_OWQQvAFoXZLdGHlObozsolNeuSxhpr84.jpg"} })
