...
const allOrder = async (req,res) => {
    try {
        ...
        // reading current user id
        let id = req.userId
            // filtering orders w.r.to current uset id
...
const singleOrder = async (req,res) => {
    try {
        ...
        // other then current user can't authorized access order details
...
const updateOrder = async (req,res) => {
    try {
        ...
        return res.status(StatusCodes.ACCEPTED).json({ status: true, msg: " order updated successfuly"})
    } catch (err) {
        return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ status: false, msg: err.message })
    }
}
...
const deleteOrder = async (req,res) => {
    try {
        ...
        return res.status(StatusCodes.ACCEPTED).json({ status: true, msg: " order deleted successfuly"})
    } catch (err) {
        return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ status: false, msg: err.message })
    }
}
